...
export async function initializeDatabase() {
  try {
    ...
    if (apiKeysError) {
      console.error('Error creating API keys table:', apiKeysError);
    }

    // Create API usage table
    const { error: apiUsageError } = await supabase.rpc('create_api_usage_table', {
      sql: `
    ...
    if (apiUsageError) {
      console.error('Error creating API usage table:', apiUsageError);
    }
